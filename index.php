<?php include 'assets/scripts/navbar.php';?>
    <div class="">
      <section id="home" class="mt-2 h-[60dvh] bg-slate-900 p-3 rounded-lg border border-slate-800">
        <div style="background-image: url('/assets/img/home-cover.jpeg');" class="bg-cover bg-center bg-no-repeat h-full rounded-lg flex flex-col items-center justify-center text-shadow-lg text-shadow-black">
          <h1 class="text-7xl">Welcome, fellow people!</h1>
          <p class="mt-2 text-xl">I am Daniel, also known as Bansku Dansku. This is my website/portfolio.</p>
        </div>
      </section>
      <section id="about" class="mt-2 h-full bg-slate-900 p-3 rounded-lg border border-slate-800 flex flex-col md:flex-row justify-between gap-4">
        <div class="md:w-2/3">
          <h1 class="text-4xl mb-2 underline decoration-3 text-violet-600"><strong>About Me</strong></h1>
          <p class="mb-2">
            I am Daniel from Finland and I mostly like tech(coding, sysadmin work, live audio, lights, video). On my freetime, I do these things as a hobby.
          </p>
          <p class="mb-2">
            I have been interested in programming for a while. I started my journey to programming in 2020 with a on the spot course about coding with Scratch, Godot, Replit, etc. There I made my first websites, python programs and Scratch games.
             After programming for six years, I have delivered 20+ websites, 6 Discord bots and 3 random apps.
          </p>
          <p class="mb-2">
            In 2024 I started learning about live audio & lights on my school. I soon became the head of technival side of festivals in my school as I mastered the digital console, Yamaha LS9 32 version, and the lighting console, Magic Quick Q10.
          </p>
          <p>
            After two years of live audio and lights, I discovered streaming/live video. I still continued on live audio & lights but started on filming TV shows and some smaller projects with my friends and colleagues using for example ATEM stuff.
          </p>
        </div>
        <img src="assets/img/banskudansku-profile.png" alt="banskudansku-profile" class="h-full hidden md:block w-1/3 object-cover rounded-full border-4 border-slate-800">
      </section>
      <section id="projects" class="mt-2 h-full bg-slate-900 p-3 rounded-lg border border-slate-800">
        <h1 class="text-6xl mb-4 underline decoration-4 text-violet-600 text-center"><strong>Projects</strong></h1>
        <div class="flex flex-row mb-4">
          <div class="grid sm:grid-cols-2 grid-row-2 gap-6 text-xl">
            <div class="flex flex-col h-full">
              <h1 class="text-4xl mb-2 underline decoration-3 text-violet-600"><strong>Axsoter</strong></h1>
              <p>Axsoter is a server hosting company founded in 2024. When Axsoter started, the primary goal was to deliver free Minecraft servers. As time has moved Axsoter is offering now premium game hosting and web hosting from Finland.
                The main selling point for Axsoter is quality price ratio and the support we offer. 
              </p>
              <div class="mt-auto">
                <hr class="border-slate-800/75 border rounded-full mb-2 mt-2">
                <i class="fa-solid fa-globe"></i><a href="https://www.axsoter.com" class="pl-2 underline decoration-2">Axsoter.com</a>
                <i class="fab fa-discord pl-2"></i><a href="https://www.axsoter.com/discord" class="pl-2 underline decoration-2">Discord</a>
              </div>
            </div>
            <img src="assets/img/axsoter.png" alt="Axsoter homepage" class="w-full h-full object-cover rounded-lg">
          </div>
        </div>
        <div class="flex flex-row mb-4">
          <div class="grid sm:grid-cols-2 grid-row-2 gap-6 text-xl">
            <img src="assets/img/etosel.png" alt="Etosel homepage" class="w-full h-full object-cover rounded-lg">
            <div class="flex flex-col h-full">
              <h1 class="text-4xl mb-2 underline decoration-3 text-violet-600"><strong>Etosel</strong></h1>
              <p>Etosel is an online workspace for schools and companies to easify their complex systems to one place in order for users to use them easily. Etosel allows users to log in to organizations sites and systems straight from one page
                without any hassle.
              </p>
              <div class="mt-auto">
                <hr class="border-slate-800/75 border rounded-full mb-2 mt-2">
                <i class="fa-solid fa-globe"></i><a href="https://www.etosel.fi" class="pl-2 underline decoration-2">Etosel.fi</a>
              </div>
            </div>
          </div>
        </div>
        <div class="flex flex-row mb-4">
          <div class="grid sm:grid-cols-2 grid-row-2 gap-6 text-xl">
            <div class="flex flex-col h-full">
              <h1 class="text-4xl mb-2 underline decoration-3 text-violet-600"><strong>Hassy Ry</strong></h1>
              <p>A funny friend group that does various things with computers and other things (see demogroup on wikipedia).
                [We/Hassy Team™/They] [have/has] done projects such as: An infoscreen for Assembly, some weirdass short films, a spinning rat orchestra, and various other things. 
              </p>
              <div class="mt-auto">
                <hr class="border-slate-800/75 border rounded-full mb-2 mt-2">
                <i class="fa-solid fa-globe"></i><a href="https://team.assyt.fi" class="pl-2 underline decoration-2">team.Assyt.fi</a>
              </div>
            </div>
            <img src="assets/img/hassyry.png" alt="HassyTeam Biisiseikkailu" class="w-full h-full object-cover rounded-lg">
          </div>
        </div>
        <div class="flex flex-row">
          <div class="grid sm:grid-cols-2 grid-row-2 gap-6 text-xl">
            <img src="assets/img/dve.png" alt="DVE homepage" class="w-full h-full object-cover rounded-lg">
            <div class="flex flex-col h-full">
              <h1 class="text-4xl mb-2 underline decoration-3 text-violet-600"><strong>DVE</strong></h1>
              <p>DVE (Digital Velocity Engineering) was born out of a passion for servers, websites, and audio, lighting, and video production.
                Although DVE operates as an official 4H company under a business license, they deliver solutions that challenge traditional services in terms of expertise and price.
              </p>
              <div class="mt-auto">
                <hr class="border-slate-800/75 border rounded-full mb-2 mt-2">
                <i class="fa-solid fa-globe"></i><a href="https://www.dve.fi" class="pl-2 underline decoration-2">Dve.fi</a>
                <i class="fab fa-discord pl-2"></i><a href="https://discord.gg/J4gwG4PmzD" class="pl-2 underline decoration-2">Discord</a>
              </div>
            </div>
          </div>
        </div>
      </section>
      <section id="contact" class="mt-2 h-full bg-slate-900 p-3 rounded-lg border border-slate-800">
        <div class="grid sm:grid-cols-2 grid-row-2 gap-6 text-xl">
          <div>
            <h1 class="text-4xl mb-2 underline decoration-3 text-violet-600"><strong>Contact</strong></h1>
            <p>Here are my contact info:</p>
            <ul>
              <li><i class="fa-regular fa-envelope mr-2"></i><a href="mailto:me@banskudansku.net" class="underline decoration-2 hover:text-violet-600 ease-out duration-400">me@banskudansku.net</a> - For personal inquiries</li>
              <li><i class="fa-regular fa-envelope mr-2"></i><a href="mailto:daniel@dve.fi" class="underline decoration-2 hover:text-violet-600 ease-out duration-400">daniel@dve.fi</a> - For business inquiries</li>
              <li><i class="fab fa-discord mr-2"></i><a href="https://discord.com/users/799296244638941264" class="underline decoration-2 hover:text-violet-600 ease-out duration-400">@banskudansku</a> - For social inquiries</li>
            </ul>            
          </div>
          <div>
            <h1 class="text-2xl mb-2 underline decoration-2 text-violet-600 text-center"><strong>Contact form</strong></h1>
            <p class="text-sm text-gray-300 text-center">You dont need to send your phone number. It is optional.</p>
            <form action="assets/scripts/contact.php" method="post" class="flex flex-col max-w-2/3 mx-auto m-2">
              <label for="email" class="pl-1 underline decoration-2">Email address</label>
              <input required type="email" name="email" id="email" placeholder="matti@meikalainen.fi" class="border-slate-800 bg-slate-950 rounded-lg border-2 h-9 placeholder:text-gray-600 p-1 focus:border-violet-600 outline-none user-invalid:border-red-500 user-invalid:text-red-500 user-invalid:focus:border-red-500">
              <label for="phone" class="mt-2 pl-1 underline decoration-2">Phone number</label>
              <input type="tel" name="phone" id="phone" placeholder="+358 12345678" class="border-slate-800 bg-slate-950 rounded-lg border-2 h-9 placeholder:text-gray-600 p-1 focus:border-violet-600 outline-none">
              <label for="text" class="mt-2 pl-1 underline decoration-2">What do you want to say?</label>
              <textarea required name="text" id="text" placeholder="I love ..." rows="4" class="border-slate-800 bg-slate-950 rounded-lg border-2 h-9 placeholder:text-gray-600 p-1 focus:border-violet-600 outline-none min-h-30 max-h-150 user-invalid:border-red-500 user-invalid:text-red-500 user-invalid:focus:border-red-500"></textarea>
              <?php if (isset($_GET['status']) && $_GET['status'] === 'success'): ?><p class="text-sm text-green-500 text-center">Form submitted succesfully</p>
              <?php elseif (isset($_GET['status']) && $_GET['status'] === 'error'): ?><p class="text-sm text-red-500 text-center">Email or text is missing or there was an error while executing.</p><?php endif; ?>
              <input type="submit" value="Submit!" class="mt-2 bg-violet-600 rounded-lg hover:bg-violet-700 active:bg-violet-800 transition duration-200 h-9">
            </form>
          </div>
        </div>
      </section>
    </div>
<?php include 'assets/scripts/footer.php';?>